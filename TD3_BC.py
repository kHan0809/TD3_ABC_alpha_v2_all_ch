...
class TD3_BC(object):
	...
	def train_v(self, replay_buffer, batch_size=256):
		self.total_it += 1
	...
	def test_epi_rv(self, replay_buffer, epi_i):
		...
		with torch.no_grad():
			...
			weight = (torch.exp((rv) / 5.0) - torch.ones_like(Return)).clamp(0.0, 10.0).mean()
		return weight.cpu().detach().numpy(), rank[0].cpu().detach().numpy()
		...
		if self.total_it % self.policy_freq == 0:
			...
			if t > clutch:
				with torch.no_grad():
					v_value = self.value(state)
					rv = Return - v_value
					weight = (torch.exp((rv) / 5.0) - torch.ones_like(Return)).clamp(0.0, 0.5)
					q_weight = (rv > 0)

				# Compute actor loss
				pi = self.actor(state)
				Q = self.critic.Q1(state, pi)*q_weight.reshape(-1,1)
