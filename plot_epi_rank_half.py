...
if __name__ == "__main__":
    ...
    if args.normalize:
        ...
        mean, std = 0, 1
    ranks, weights, zero_nums = [], [], []
    for i in range(epi_len-1):
        weight,rank,zero_num=policy.test_epi_rv_rank(replay_buffer,i)
        weights.append(weight)
        ranks.append(rank)
        zero_nums.append(zero_num)
    weights = np.array(weights)
    ranks   = np.array(ranks)
    zero_nums = np.array(zero_nums)
...
#     ranks = ranks[idx]
#     weights = weights[idx]
#     plt.plot(ranks, weights, 'o',color='skyblue')
#     plt.plot(ranks, zero_nums, 'o', color='pink')
#
#     x = np.array([i * 25 for i in range(1,epi_len // 25 + 1)])
#     interval_weight = []
#     for i in range((epi_len//25)-1):
